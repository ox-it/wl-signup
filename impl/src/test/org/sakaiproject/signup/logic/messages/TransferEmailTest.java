...
public class TransferEmailTest {
    ...
    public void setUp() {
        ...
        when(_mockedCalendarHelper.generateExtEventForTimeslot(_mockedMeeting, _mockedTimeslot)).thenReturn(_mockedEvent);
    }
    ...
    private void assertGenerates(int n) {
        final List<ExtEvent> events = _email.generateEvents(_mockedAttendingUser1, _mockedCalendarHelper);
        assertEquals(n, events.size());
    }
    
    private void assertCancels(int n) {
        verify(_mockedCalendarHelper, times(n)).cancelExtEvent(_mockedEvent);
    }
}
